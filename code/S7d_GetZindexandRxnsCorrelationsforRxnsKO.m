...
%%
% Load Data and Mutant Labels
sample = 'C' ;
load(fullfile(results_dir_name, [sample '_DynMOMAsingleRxns.mat']))
[resultData_C, zd_log_norm_C, mutant_labels_C]  = processReactionData(resultData);
sample = 'D' ;
load(fullfile(results_dir_name, [sample '_DynMOMAsingleRxns.mat']))
[resultData_D, zd_log_norm_D, mutant_labels_D]  = processReactionData(resultData);
...
fig_name_plot_png = fullfile(dir_name,  'Fig_7d_RxnKORxnCorrPosNeg.svg');
saveas(gcf, fig_name_plot_png);
%  Plot Z index
trunc_id = 1 ;reactionNames = mutant_labels_C;
% Combine the data for stacked bar plot
...
fontSize = 16; 
set(gca,'FontWeight', 'bold', 'FontSize', fontSize, 'FontName', 'Helvetica', 'Box', 'off');
fig_name_plot_png = fullfile(dir_name,  'Fig_7c_RxnKO_Zindex.png');
saveas(gcf, fig_name_plot_png);
...
function [resultData, zd_log_norm, mutant_labels] = processReactionData(resultData)
    % Extract necessary data and labels from resultData structure
    mutant_labels = resultData.SampleName_all;
    mutant_labels{2} = 'SPT';
