...
% DMFA function call with updated variable names
...
% **DMFASampler*
% % **Description:**
% DMFASampler is a MATLAB function that performs dynamic metabolic flux analysis (DMFA) on a given metabolic model using concentration data. 
...
%     modelTypes(measuredMetaboliteIndices([2 4 7 8 23 24 31 32 36])) = {'bal'};
...
%% ALTERNATIVE CONSTRAINED OPTIMIZATION using baron to obtain global solution
    %% Constraints
...
%     % SLD (c&d) - C24
%     constrainedRxnIdx =  [1 12 13] ; 
%     Rxnbounds_lb = [0 0 0] ;
%     Rxnbounds_ub = [inf 0 0] ;
% 
% 
%     % SLD (a&b) - C16
%     constrainedRxnIdx =  [1 10 11] ; 
%     Rxnbounds_lb = [0 0 0] ;
%     Rxnbounds_ub = [inf 0 0] ;
% 
%     % SLD (b&d) - t18:0
%     constrainedRxnIdx =  [1 11 12] ; 
%     Rxnbounds_lb = [0 0 0] ;
%     Rxnbounds_ub = [inf 0 0] ;
% 
% 
%     % SLD (a&c) - d18:0
%     constrainedRxnIdx =  [1 10 13] ; 
%     Rxnbounds_lb = [0 0 0] ;
%     Rxnbounds_ub = [inf 0 0] ;
% 
% 
%     % CS I&II (b&b) - t18:0
%     constrainedRxnIdx =  [1 7 9] ; 
%     Rxnbounds_lb = [0 0 0] ;
%     Rxnbounds_ub = [inf 0 0] ;
% 
% 
%     % CS I&II (a&a) - d18:0
%     constrainedRxnIdx =  [1 6 8] ; 
%     Rxnbounds_lb = [0 0 0] ;
%     Rxnbounds_ub = [inf 0 0] ;
% 
%     % CS II (a&b) - C24
%     constrainedRxnIdx =  [1 8 9] ; 
%     Rxnbounds_lb = [0 0 0] ;
%     Rxnbounds_ub = [inf 0 0] ;
% 
% 
%     % CS I (a&b) - C16
%     constrainedRxnIdx =  [1 6 7] ; 
%     Rxnbounds_lb = [0 0 0] ;
%     Rxnbounds_ub = [inf 0 0] ;
% 
%     % CS (I&II)  - All CSs
%     constrainedRxnIdx =  [1 6 7 8 9] ; 
%     Rxnbounds_lb = [0 0 0 0 0] ;
%     Rxnbounds_ub = [inf 0 0 0 0] ;
% 
%     % LCBK (a&b)
%     constrainedRxnIdx =  [1 4 5] ; 
%     Rxnbounds_lb = [0 0 0] ;
%     Rxnbounds_ub = [inf 0 0] ;
% 
%     % SBH 
%     constrainedRxnIdx =  [1 3] ; 
%     Rxnbounds_lb = [0 0 ] ;
%     Rxnbounds_ub = [inf 0] ;
%       
% 
...
%     Rxnbounds_ub = [inf] ;
    ...
    %% Baron solver usage
    % Define the options for BARON
%     options = baronset('MaxTime', 1000, ); % Set options like maximum time, tolerance, etc.
...
%     [p,fval,exitflag,output] = baron(objM, [], [], [], [], [], [], [], [], [] , NaN(size(H, 2), 1), baron_opts) ; %,  [], [], [], [], options);

    
    function col_vector = nlcon_fun(x, j_vec, K, nc, nu, nt)
    % This function returns a column vector based on the vector of 'j' indices and input 'x'
        % Reshape 'x' into a matrix of size (nu, nt)
