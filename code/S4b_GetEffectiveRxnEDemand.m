...
for i = 1:size(met_list_X_, 1)
    if ~all(isnan(tmp_data1(i,:))) && ~all(isnan(tmp_data2(i,:)))
        ...
        tmp_x =  trimOutliers(Econc_1(i,:)) ;
        tmp_y =  trimOutliers(Econc_2(i,:)) ;
       ...
       median_data_e(i, :) = [median(tmp_x), median(tmp_y)] ;
      [h_values_e(i, 1), p_values_e(i, 1)] = kstest2(tmp_x, tmp_y , 'Alpha', 0.01) ;
    else 
%         i
    end
end
...
xlabel('Rxn IDs');
title('Distribution of Relative Enzyme Costs Specificity');
...
fig_name_plot_png = fullfile(dir_name, 'Fig_5b_EffRxn_ECostDist.png'); 
